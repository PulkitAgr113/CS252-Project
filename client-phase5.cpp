...
int main(int argc, char** argv){
    ...
    for(int nf = 0; nf < filenames_2.size(); nf ++){
        ...
        if(unique_id_neigh > 0){
            neighbors_depth2.insert(to_string(unique_id_neigh));
            depth[index] = 2;
    ...
    no_of_connected_neighbors_lesser = 0;
    no_of_connected_neighbors_greater = 0;
    set <pair <string, string> > new_neighbors;
    set <pair <string, string> > neighbor_data_to_provide;
    ...
    while (no_of_connected_neighbors_lesser < no_of_neighbors_lesser) {
        ...
        no_of_connected_neighbors_lesser ++;
        string num_files_2 = to_string(neighbors_depth2.size());
        char buffer[100] = {0};
        char num_neighbor_files[100]={0};
        ...
        for(int nf = 0; nf < strtol(num_neighbor_files, nullptr, 10); nf ++) {
            char unique_id_asked[100]={0};
            read(new_fd, unique_id_asked, 100);
            auto it = find_if(neighbor_initial_data.begin(), neighbor_initial_data.end(), [&](const tuple<int, string, string>& e) {return get<2>(e) == (string) unique_id_asked;});
            if (it != neighbor_initial_data.end()){
                int index=it-neighbor_initial_data.begin();
                string to_send = to_string(get<0>(neighbor_initial_data[index]));
                send(new_fd, &to_send[0], to_send.length(), 0);
                read(new_fd, random_read, 100);
                to_send = get<1>(neighbor_initial_data[index]);
                send(new_fd, &to_send[0], to_send.length(), 0);
                neighbor_data_to_provide.insert({(string) unique_id_asked, (string) buffer});
            } 
            else{
                send(new_fd, "N", 1, 0);
                read(new_fd, random_read, 100);
        ...
        for(auto unique_id_toask: neighbors_depth2 ) {
            char clientno_recv[100]={0};
            char port_recv[100]={0};

            send(new_fd, &unique_id_toask[0],unique_id_toask.length(),0);
            read(new_fd, clientno_recv,100);
            send(new_fd, &random_send[0], random_send.length(), 0);
            read(new_fd, port_recv,100);

            if((string) clientno_recv != "N"){
                new_neighbors.insert({(string) clientno_recv, (string) port_recv});
            }
        }
    ...
    for (int neigh = 0; neigh < no_of_neighbors; neigh ++ ) {
        ...
        for(string unique_id_toask: neighbors_depth2 ) {
            char clientno_recv[100]={0};
            char port_recv[100]={0};

            send(neighbor_sockfd, &unique_id_toask[0],unique_id_toask.length(),0);
            read(neighbor_sockfd, clientno_recv,100);
            send(neighbor_sockfd, &random_send[0], random_send.length(), 0);
            read(neighbor_sockfd, port_recv,100);
        ...
        for(int nf = 0; nf < strtol(num_neighbor_files, nullptr, 10); nf ++) {
            ...
            read(neighbor_sockfd, unique_id_asked, 100);
            auto it = find_if(neighbor_initial_data.begin(), neighbor_initial_data.end(), [&](const tuple<int, string, string>& e) {return get<2>(e) == (string) unique_id_asked;});
            if (it != neighbor_initial_data.end()){
                int index=it-neighbor_initial_data.begin();
                string to_send=to_string(get<0>(neighbor_initial_data[index]));
                send(neighbor_sockfd, &to_send[0], to_send.length(), 0);
    ...
    while (no_of_connected_neighbors_greater < no_of_neighbors_greater) {
        ...
        char buffer[100] = {0};
        char num_neighbor_files[100]={0};
        vector <string> current_unique_ids;
    
        read(new_fd, buffer, 100);
        send(new_fd, &unique_id[0], unique_id.length(), 0);
        
        for(auto t: neighbor_data_to_provide) {
            if(t.first == (string) buffer) current_unique_ids.push_back((string) t.second);
        }
        string num_files_2 = to_string(current_unique_ids.size());
        ...
        for(string unique_id_toask: current_unique_ids ) {
            
            auto it = find_if(neighbor_initial_data.begin(), neighbor_initial_data.end(), [&](const tuple<int, string, string>& e) {return get<2>(e) == unique_id_toask;});
            int index = it-neighbor_initial_data.begin();
            string to_send = to_string(get<0>(neighbor_initial_data[index]));
            send(new_fd, &to_send[0], to_send.length(), 0);
            read(new_fd, random_read, 100);
    ...
    for(auto t: new_neighbors) {
        //cout<<t.first<<" "<<t.second<<endl;
        neighbors.push_back(t);
        string neighbor_no = t.first;
        if(stoi(neighbor_no) < stoi(client_no)) no_of_neighbors_lesser ++;
        else no_of_neighbors_greater ++;
        no_of_neighbors++;
    }
    ...
    while (no_of_connected_neighbors_lesser < no_of_neighbors_lesser) {
        ...
        for(int nf = 0; nf < stoi(no_of_files); nf ++) {
            if(strtol(buffer, nullptr, 10) == final_unique_id[nf]) files_wanted.push_back(files[nf]);
        }
    ...
    for(int nf = 0; nf < stoi(no_of_files); nf ++) {
        cout << "Found " << files[nf] << " at " << final_unique_id[nf] << " with MD5 " << MD5[nf] << " at depth " << depth[nf] << "\n";
    }
